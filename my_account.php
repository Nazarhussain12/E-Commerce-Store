   ...
   <?php if(isset($_SESSION['customer_email'])): ?>
    ...
    <div class="content_wrapper">
        <div id="sidebar">
            ...
            <ul class="cats">
            <?php
$user = $_SESSION['customer_email'] ?? null;
$c_image = './images/empty.webp'; // Path to default image
$c_name = 'Guest';
...
if ($user) {
    ...
    $run_img = mysqli_query($con, $get_img);
    $row_img = mysqli_fetch_array($run_img);
    if ($row_img && file_exists("customer/customer_images/" . $row_img['cust_image'])) {
        $c_image = "customer/customer_images/" . $row_img['cust_image'];
        $c_name = $row_img['cust_name'];
    }
}

echo "<img src='" . htmlspecialchars($c_image) . "' style ='border: 2px solid white; border-radius: 50%; cursor: pointer; width: 180px; height: 170px; margin-right: 10px;' alt='" . htmlspecialchars($c_name) . "'>";
?>
                ...
                <span >
                    <?php
                        if(isset($_SESSION['customer_email'])){
                            ...
                            echo "<a style='color: orange;' href='../logout.php'></a>";
                        } else {
                        ...
                        if (!isset($_GET['edit_account'])) {
                            if (!isset($_GET['change_pass'])) {
                                if (!isset($_GET['del_account'])) {
                                    echo "<h2 style='font-weight:bold; font-size:50px;'> Welcome:  $c_name </h2> <br><br><br><br>";
                                    echo "<br><br><b><a href='./my_account.php?my_orders'> See Order's Progress </a></b>";
                                }
                            }
                        }
                    }
